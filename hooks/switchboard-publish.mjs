// Claude Code PostToolUse/Stop hook → auto-publishes this agent's activity to the switchboard.
// Fire-and-forget, fast timeout, silent on any failure (never blocks the session).
// Install via settings.json; reads ~/.switchboard/config.json for { base, token, agent_id }.
